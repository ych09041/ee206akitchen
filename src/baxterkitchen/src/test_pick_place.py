...
def main():
    ...
    angles = raw_input("state vars: ")
    start_array = np.array(angles.split(), dtype='|S4')
    start_arrayy = start_array.astype(np.float)
    goal.request.start_state.joint_state.position = start_arrayy
    ...
    angles = raw_input("target position: ")
    start_array = np.array(angles.split(), dtype='|S4')
    start_arrayy = start_array.astype(np.float)
    target_joint_angles = start_arrayy
